                      ...
                      <n-card size="small">
                        <template #header>
                          <n-flex justify="space-between" :size="[5, 0]">
                            ...
                            <n-flex>
                              <n-button size="tiny" @click="batRowAction(item.table, tIdx, 'cleanBorder')"> 批量去边框 </n-button>
                              <n-button size="tiny" @click="batRowAction(item.table, tIdx, 'addBold')"> 批量加粗 </n-button>
                                  ...
                                  <n-flex>
                                    ...
                                    <div>
                                      <n-input-group>
                                        ...
                                        <n-input-number v-model:value="body.fontSize" size="tiny" placeholder="字体大小" style="width: 70px" />
                                      </n-input-group>
                                    </div>
                                  </n-flex>
                                </n-flex>
                            ...
                            <n-flex :size="[5, 2]" class="m-t-xs">
                              ...
                              <div>
                                <n-popover trigger="hover">
                                  ...
                                  <n-flex :size="[5, 0]">
                                    自定义:<n-checkbox v-for="(_, bIndex) in body.border" v-model:checked="body.border[bIndex]" @update-checked="preview">
                                      {{ position[bIndex] }}
                                    </n-checkbox>
                                  </n-flex>
                                  <n-flex :size="[5, 0]">
                                    仅保留:
                                    <n-button size="tiny" @click="setBorder('lr', body)">左右</n-button>
                                    <n-button size="tiny" @click="setBorder('tb', body)">上下</n-button>
                                  </n-flex>
                                </n-popover>
                              </div>
...
const batRowAction = (table, idx, action: 'cleanBorder' | 'addBold') => {
  const cols = table.body[idx];
  for (const item of cols) {
    if (action == 'addBold') item.bold = true;
    if (action == 'cleanBorder') item.border = [false, false, false, false];
  }
  preview();
};
...
// 延迟触发,否则v-model会丢失数据
const onDomClick = preview;

const setBorder = (type: 'lr' | 'tb', body) => {
  if (type == 'lr') body.border = [true, false, true, false];
  if (type == 'tb') body.border = [false, true, false, true];
  preview();
};
...
let bakCurrentPage = 0;
docDefinition.value.header = function (currentPage, pageCount, pageSize) {
  if (currentPage > bakCurrentPage) {
    // // addHeader()
    // docDefinition.value.content.push({
    //   table: {
    //     tittle: '头部',
...
onMounted(() => {
  docDefinition.value.pageMargins = [40, 206, 40, 20];
  docDefinition.value.background = [
    {
      table: {
        tittle: '头部',
        body: [
