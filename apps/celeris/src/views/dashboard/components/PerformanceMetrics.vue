...
use([CanvasRenderer, LegendComponent, RadarChart, BarChart]);
...
const { chartOption } = useChartOption((isDark: boolean) => {
  return {
    ...
    radar: {
      center: ['35%', '50%'],
      radius: 80,
      indicator: [{ name: 'API调用次数' }, { name: '响应时间' }, { name: '错误率' }, { name: '服务器负载' }, { name: '用户满意度' }, { name: '消息处理速度' }],
      axisName: {
...
console.log('chartOption log==>', chartOption);
</script>

<template>
  <!--  清晰地传达该模块关注的是Chatbot的技术性能方面的指标。 -->
  <NCard title="技术指标综述">
    <VChart style="height: 222px" :option="chartOption" autoresize />
  </NCard>
</template>
