...
const { chartOption } = useChartOption((isDark: boolean) => {
  return {
    ...
    series: [
      generateSeries('用户数', '#722ED1', '#F5E8FF', userData.value),
      generateSeries('总对话数', '#F77234', '#FFE4BA', totalConversationData.value),
      generateSeries('被复制对话数', '#33D1C9', '#E8FFFB', copiedConversationData.value),
      generateSeries('点赞数', '#3469FF', '#E8F3FF', likedResponsesData.value),
    ],
  };
});
