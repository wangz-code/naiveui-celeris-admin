...
function getGenerateColors(color: string, darkMode: boolean): string[] {
  return darkMode ? generateColorPalettes(color, true, commonDark.bodyColor) : generateColorPalettes(color);
}
 ...
 */
function getOtherColor(config: ThemeConfig, darkMode: boolean): GlobalThemeOverrides {
  ...
  const scenes: ButtonColorScene[] = ['', 'Hover', 'Pressed', 'Focus', 'Disabled'];
  keys.forEach((key) => {
 ...
 */
export function getThemeColors(config: ThemeConfig, darkMode: boolean): ThemeColor {
 ...
 */
export function getNaiveUICustomTheme(config: ThemeConfig, darkMode: boolean): GlobalThemeOverrides {
