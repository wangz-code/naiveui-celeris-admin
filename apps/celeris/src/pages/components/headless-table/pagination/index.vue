...
<template>
	<NCard title="分页表单">
		<NTable :bordered="true" :single-line="false" striped>
			<thead>
				<tr v-for="headerGroup in table.getHeaderGroups()" :key="headerGroup.id">
					<th v-for="header in headerGroup.headers" :key="header.id" :colspan="header.colSpan" class="font-bold!" scope="col">
		...
		<template #action>
			<NPagination :page="table.getState().pagination.pageIndex + 1" :page-count="table.getPageCount()" show-size-picker :page-sizes="[10, 20, 30, 40, 50]" @update:page="handlePageChange" @update:page-size="handlePageSizeChange" />
		</template>
	</NCard>
</template>
