...
const { chartOption } = useChartOption((isDark: boolean) => {
	return {
		...
		series: [generateSeries("用户数", "#722ED1", "#F5E8FF", userData.value), generateSeries("总对话数", "#F77234", "#FFE4BA", totalConversationData.value), generateSeries("被复制对话数", "#33D1C9", "#E8FFFB", copiedConversationData.value), generateSeries("点赞数", "#3469FF", "#E8F3FF", likedResponsesData.value)],
	};
});
		...
		<NCard title="数据总览">
			<NRow justify="space-between">
				<NCol v-for="(item, idx) in renderData" :key="idx" :span="6">
					<NStatistic :label="item.title" tabular-nums>
						<span class="inline-block w-8 h-8 text-center align-baseline flex-center rounded mr-2" :style="{ background: item.prefix.background }">
							<Iconx :size="20" :component="item.prefix.icon" :color="item.prefix.iconColor" />
						</span>
						<span>{{ item.value }}</span>
					</NStatistic>
				</NCol>
			</NRow>
			<VChart style="height: 320px; margin-top: 10px" :option="chartOption" autoresize />
		</NCard>
	</NSpin>
</template>
