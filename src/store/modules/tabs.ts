...
export const useTabsStore = defineStore({
	...
	getters: {
		...
		getLimitTabsList(state): Tab[] {
			...
			return takeRight(state.tabs.filter((tab) => state.pinnedTabs.findIndex((p) => p.fullPath === tab.fullPath) === -1).reverse(), state.maxVisibleTabs);
		},
	...
	actions: {
		...
		close(isPinned: boolean, tab: Tab) {
			if (isPinned) this.pinnedTabs = this.pinnedTabs.filter((currentTab) => currentTab.fullPath !== tab.fullPath);
